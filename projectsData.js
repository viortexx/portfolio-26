const projectsData = {
  "WFI": {
    "title": "Women for Independence",
    "credits": [
      { "label": "Client", "value": "Tv Cultura" },
      { "label": "Motion design", "value": "Vitória Coelho" },
      { "label": "Illustrations", "value": "Marina Quintanilha" },
      { "label": "Production", "value": "Multiverso Produções" },
      { "label": "Direction", "value": "Nina Kopko" }
    ],
    "description": "“Women for Independence” is a 5-episode documentary series for TV about the five women who played an important and active role in the process of Independence in Brazil: Maria Quitéria, Joana Angélica, Maria Felipa, Maria Leopoldina and Bárbara de Alencar.",
    "media": [
      { "type": "image", "src": "./ASSETS/PROJECTS/Women-for-independency/clip_All-Women.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Women-for-independency/clip1_WFI.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Women-for-independency/clip2_WFI.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Women-for-independency/clip3_WFI.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Women-for-independency/clip4_WFI.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Women-for-independency/clip5_WFI.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Women-for-independency/clip6_WFI.gif" }
    ]
  },
  "Cigana-Lyrics": {
    "title": "Cigana | Baco Exu do Blues",
    "credits": [
      { "label": "Motion Design", "value": "Vitória Coelho" },
      { "label": "3D handmodel", "value": "SketchFab" },
      { "label": "Song", "value": "Cigana by Baco Exu do Blues" }
    ],
    "description": "Lyric video project for the song Cigana by Baco Exu do Blues.",
    "media": [
      { "type": "video", "src": "./ASSETS/PROJECTS/Cigana-Lyrics/Cigana-Lyrics_BACOEXU_1.mp4" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Cigana-Lyrics/Still1_Cigana.webp" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Cigana-Lyrics/Still2_Cigana.webp" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Cigana-Lyrics/Still3_Cigana.webp" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Cigana-Lyrics/Still4_Cigana.webp" }
    ]
  },
  "Whales": {
    "title": "Whales of the Brazilian Coast",
    "credits": [
      { "label": "Client", "value": "g1 (Globo TV)" },
      { "label": "Motion Design", "value": "Vitória Coelho" },
      { "label": "Illustrations", "value": "Luisa Rivas" },
      { "label": "Production and voiceover", "value": "Mariana Mendicelli" }
    ],
    "description": "This is a video made with the Art Department of g1 news portal (part of brazilian TV channel GloboTV) about the migration and annual appearance of humpback and right whales around the brazilian coast.",
    "media": [
      { "type": "video", "src": "./ASSETS/PROJECTS/Whales-of-brazilian-coast/As gigantes da costa brasileira_1.mp4" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Whales-of-brazilian-coast/Whales_1.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Whales-of-brazilian-coast/Whales_2.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Whales-of-brazilian-coast/Whales_3.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Whales-of-brazilian-coast/Whales_4.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Whales-of-brazilian-coast/Whales2_1.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Whales-of-brazilian-coast/Whales2_2.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Whales-of-brazilian-coast/Whales2_3.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Whales-of-brazilian-coast/Whales2_4.gif" }

    ]
  },
  "NFL": {
    "title": "NFL Superbowl XL",
    "credits": [
      { "label": "Client", "value": "geTV" },
      { "label": "Art Direction", "value": "Vitória Coelho" },
      { "label": "Motion Design", "value": "Thales Cardoso" },
      { "label": "Production", "value": "Dave Avigdor" }
    ],
    "description": "I worked as the sole Art Director for geTV (part of Globo TV's network), creating this and many other advertisements to promote their brand-new YouTube channel. This particular piece was created to announce their broadcast of Super Bowl XL.",
    "media": [
      { "type": "video", "src": "./ASSETS/PROJECTS/getv-superbowl-xl/Masthead_NFL_1.mp4" },
      { "type": "image", "src": "./ASSETS/PROJECTS/getv-superbowl-xl/still1_NFL-Masthead.webp" },
      { "type": "image", "src": "./ASSETS/PROJECTS/getv-superbowl-xl/still2_NFL-Masthead.webp" },
      { "type": "image", "src": "./ASSETS/PROJECTS/getv-superbowl-xl/still3_NFL-Masthead.webp" },
      { "type": "image", "src": "./ASSETS/PROJECTS/getv-superbowl-xl/still4_NFL-Masthead.webp" },
      { "type": "image", "src": "./ASSETS/PROJECTS/getv-superbowl-xl/still5_NFL-Masthead.webp" },
      { "type": "image", "src": "./ASSETS/PROJECTS/getv-superbowl-xl/still6_NFL-Masthead.webp" }
    ]
  },
  "American-Elections": {
    "title": "2024 American Elections",
    "credits": [
      { "label": "Client", "value": "g1 (Globo TV)" },
      { "label": "Art Direction and Motion Design", "value": "Vitória Coelho" },
      { "label": "Production", "value": "Guilherme Luiz Pinheiro" }
    ],
    "description": "Every election year, G1 News covers the North American Election with insights, explainers and live videos. On the year of 2024 I was responsible for creating all the concept and animations, as well as some social media and infographic templates, for the coverage that is headed way before the voting day.",
    "media": [
      { "type": "image", "src": "./ASSETS/PROJECTS/g1-American-Elections/Opening_AmericanElections.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/g1-American-Elections/Trump_AmericanElections.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/g1-American-Elections/JoeBiden_AmericanElections.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/g1-American-Elections/Wipe_AmericanElections.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/g1-American-Elections/Blue_Lowerthird_AmericanElections.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/g1-American-Elections/Red_Lowerthird_AmericanElections.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/g1-American-Elections/Background_AmericanElections.gif" }
    ]
  },
  "Stormwind": {
    "title": "Stormwind Studios",
    "layout": "2x2-autoplay",
    "credits": [
      { "label": "Client", "value": "StormWind Studios" },
      { "label": "Art Direction and Motion Design", "value": "Vitória Coelho" },
      { "label": "Production", "value": "Letícia Zampieri" }
    ],
    "description": "StormWind Studios reached out to me to create four software demonstration videos for their new website, showcasing parts of their learning platform and their new AI agent.",
    "media": [
      { "type": "video", "src": "./ASSETS/PROJECTS/Stormwind-Studios/STORMWIND_Vid1.mp4" },
      { "type": "video", "src": "./ASSETS/PROJECTS/Stormwind-Studios/STORMWIND_Vid2.mp4" },
      { "type": "video", "src": "./ASSETS/PROJECTS/Stormwind-Studios/STORMWIND_Vid3.mp4" },
      { "type": "video", "src": "./ASSETS/PROJECTS/Stormwind-Studios/STORMWIND_Vid4.mp4" }
    ]
  },
  "Safespace": {
    "title": "Safespace",
    "credits": [
      { "label": "Client", "value": "SafeSpace Platform" },
      { "label": "Motion Design", "value": "Vitória Coelho" },
      { "label": "Illustration", "value": "Gabriel Wesley" },
      { "label": "Production", "value": "Rafaela Frankenthal" },
    ],
    "description": "SafeSpace is a technology company that operates in the fields of Compliance and Human Resources with the purpose of being an ally in creating ethical, safe, and inclusive work environments. They approched me to collaborate with their SafeLearning Academy training videos.",
    "media": [
      { "type": "video", "src": "./ASSETS/PROJECTS/Safespace/MONEYLAUNDRY-Video_SAFESPACE_1.mp4" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Safespace/Opening_SAFESPACE-2.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Safespace/MoneyLaundry_SAFESPACE-.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Safespace/Part1_Pression_SAFESPACE.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Safespace/Robert_SAFESPACE.gif" }
    ]
  },
  "Heartchain": {
    "title": "Heartchain Visuals",
    "layout": "hero-left",
    "credits": [
      { "label": "Motion Design", "value": "Vitória Coelho" },
      { "label": "3D stage scenario", "value": "Guilherme de Oliveira Sousa" }
    ],
    "description": "These visuals are part of a personal project, inspired by a friend's tattoo. They were created to be like a set of screens that could be used for concerts.",
    "media": [
      { "type": "image", "src": "./ASSETS/PROJECTS/Heartchain-Screens/HEARTCHAIN_Vertical_2.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Heartchain-Screens/HEARTCHAIN_Wide-2.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Heartchain-Screens/HEARTCHAIN_Palco.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Heartchain-Screens/HEARTCHAIN_3DTriple.gif" },
      { "type": "image", "src": "./ASSETS/PROJECTS/Heartchain-Screens/HEARTCHAIN_3DSolo_1.gif" }
    ]
  },
  "Maneskin": {
    "title": "I Wanna be Your Slave | Maneskin",
    "credits": [
      { "label": "Art Direction and Motion Design", "value": "Vitória Coelho" },
      { "label": "Song", "value": "I Wanna Be Your Slave by Maneskin" }
    ],
    "description": "This is an experimental lyric video made for personal studies purposes only.",
    "media": [
      { "type": "video", "src": "./ASSETS/PROJECTS/I-wanna-be-your-slave-lyrics/Lyric video_Maneskin_1.mp4" },
      { "type": "image", "src": "./ASSETS/PROJECTS/I-wanna-be-your-slave-lyrics/Still1_MANESKIN.webp" },
      { "type": "image", "src": "./ASSETS/PROJECTS/I-wanna-be-your-slave-lyrics/Still2_MANESKIN.webp" },
      { "type": "image", "src": "./ASSETS/PROJECTS/I-wanna-be-your-slave-lyrics/Still3_MANESKIN.webp" },
      { "type": "image", "src": "./ASSETS/PROJECTS/I-wanna-be-your-slave-lyrics/Still4_MANESKIN.webp" }
    ]
  }
};
